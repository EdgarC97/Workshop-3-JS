...
let updateProduct = () => {
  ...
  if (product) {
    let fields = prompt(
      "Ingrese los campos del producto que desea modificar (name, price, quantity, description), separados por comas:"
    );
...
function sortProducts() {
  let property = prompt("¿Quieres ordenar por: \n 1:Precio \n 2.Cantidad");
  let order = prompt("¿Quieres ordenar de forma:\n 1.Ascendente \n 2.Descendente");

  // Convertir la opción del usuario a la propiedad correspondiente
  property = property === "1" ? "price" : "quantity";

  if (property === "price" || property === "quantity") {
    if (order === "1") {
      let productosOrdenados = products.sort((a, b) => a[property] - b[property]);
      console.log(productosOrdenados);
    } else if (order === "2") {
      let productosOrdenados = products.sort((a, b) => b[property] - a[property]);
      console.log(productosOrdenados);
    } else {
      console.log("Opcion invalida. Por favor, introduce '1' o '2'.");
    }
  } else {
    console.log("Opcion invalida. Por favor, introduce '1' o '2'.");
  }
...
let generateProductReport = () => {
  ...
  let mostAvailableProductsCount = products.filter(
    (product) => product.quantity === mostAvailableProductQuantity
  ).length;
  ...
  let leastAvailableProductsCount = products.filter(
    (product) => product.quantity === leastAvailableProductQuantity
  ).length;
